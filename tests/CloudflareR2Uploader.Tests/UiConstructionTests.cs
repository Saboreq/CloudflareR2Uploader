...
namespace CloudflareR2Uploader.Tests
{
    ...
    public sealed class UiConstructionTests
    {
        ...
        public void MainForm_BrowserLayoutScalesAtCommonDpiFactors()
        {
            ...
            Thread thread = new Thread(() =>
            {
                try
                {
                    ...
                    foreach (float factor in factors)
                    {
                        using (MainForm form = new MainForm(null))
                        {
                            // Hosted Windows runners can expose a 1024x768 desktop. Keep this
                            // layout test independent of the host screen's working area.
                            form.StartPosition = FormStartPosition.Manual;
                            form.Location = new Point(-32000, -32000);
                            form.CreateControl();
                            if (factor != 1.0f) form.Scale(new SizeF(factor, factor));
                            form.ClientSize = new Size(
                                (int)Math.Ceiling(1120 * factor),
                                (int)Math.Ceiling(850 * factor));
                            form.PerformLayout();
                            ...
                            if (!layoutValid)
                            {
                                layoutFailure = string.Format(
                                    "factor={0}; form={1}; minimum={2}; browser={3}; grid={4}; refresh={5}; up={6}",
                                    factor,
                                    form.ClientSize,
                                    form.MinimumSize,
                                    browserPanel == null ? "missing" : browserPanel.Size.ToString(),
                                    grid == null ? "missing" : grid.Size.ToString(),
                                    refresh == null ? "missing" : refresh.Bounds.ToString(),
                                    up == null ? "missing" : up.Bounds.ToString());
                                break;
                            }
                        }
                    }
            ...
            Assert.IsNull(layoutFailure, "The browser layout collapsed at a common Windows DPI scale: " + layoutFailure);
        }
