...
namespace CloudflareR2Uploader.Tests
{
    ...
    public sealed class BrowserViewServiceTests
    {
        ...
        [TestMethod] public void Filter_IsCaseInsensitiveAndMatchesTypeExtensionAndKey()
        {
            ...
            CollectionAssert.AreEqual(new[] { "folder/" }, Service.Apply(items, "FOLDER", BrowserSortColumn.Name, BrowserSortDirection.Ascending).Items.Select(x => x.Identity).ToArray());
            CollectionAssert.AreEqual(new[] { "z/photo.JPG" }, Service.Apply(items, "image", BrowserSortColumn.Name, BrowserSortDirection.Ascending).Items.Select(x => x.Identity).ToArray());
            CollectionAssert.AreEqual(new[] { "z/photo.JPG" }, Service.Apply(items, "jpg", BrowserSortColumn.Name, BrowserSortDirection.Ascending).Items.Select(x => x.Identity).ToArray());
