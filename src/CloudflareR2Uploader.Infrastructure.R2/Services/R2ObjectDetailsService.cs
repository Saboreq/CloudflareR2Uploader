...
namespace CloudflareR2Uploader.Services
{
    public sealed class R2ObjectDetailsService
    {
        ...
        public static void AddExtractedMetadata(R2ObjectProperties properties, string localPath)
        {
            if (properties == null || string.IsNullOrEmpty(localPath)) return;
            try
            {
                string[] useful = { "Image Width", "Image Height", "Make", "Model", "Date/Time Original", "Orientation", "Color Space", "Color Type", "Compression Type", "Format" };
